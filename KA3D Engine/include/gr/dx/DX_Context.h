...
public:
	...
	/** Shader::ParamType as String objects. */
	lang::Array<lang::String>		shaderparamname;
	
	/** Global list of all shader objects. */
	lang::LinkedList<DX_ContextItem> itemlist;

	/** Transform buffer for transformations needed in polygon sorting. */
	lang::Array<math::float4x4>		boneworldtm;

	/** Cached model-world transform, used for sorting. */
	math::float4x4					worldtm;
	 ...
	 */
	Texture*	createTexture( const lang::String& filename );
	 ...
	 * @exception GraphicsException
	 */
	CubeTexture* createCubeTexture( const lang::String& filename );
	 ...
	 * @exception GraphicsException
	 */
	Shader*		createShader( const lang::String& name, int flags );
	 ...
	 * @exception IOException
	 */
	void		capture( const lang::String& namefmt );
	 ...
	 */
	const math::float4x4&	screenTransform() const;
...
private:
	...
	D3DCAPS9				m_caps;
	lang::Hashtable<lang::String,P(gr::Shader)>	m_shaders;
	lang::Array<P(DX_Primitive)>				m_dynamicPrimitives;
	int						m_capturenum;

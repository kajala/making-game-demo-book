...
public:
	 ...
	 */
	explicit DX_Shader( DX_Context* context, const lang::String& name, 
		const lang::String& fxfilename, int flags );
	 ...
	 */
	void	setMatrix( ParamType param, const math::float4x4& value );

	/**
	 * Sets custom shader 4x4 matrix parameter which is not defined in ParamType.
	 */
	void	setMatrix( const char* name, const math::float4x4& value );

	/**
	 * Sets shader 4x4 matrix array parameter.
	 */
	void	setMatrixArray( ParamType param, math::float4x4** values, int count );

	/**
	 * Sets shader 4-vector parameter.
	 */
	void	setVector( ParamType param, const math::float4& value );

	/**
	 * Sets custom shader 4-vector parameter which is not defined in ParamType.
	 */
	void	setVector( const char* name, const math::float4& value );
	 ...
	 */
	math::float4x4	getMatrix( const char* name );

	/**
	 * Gets shader vector parameter or 0 if texture not set.
	 */
	math::float4	getVector( const char* name );

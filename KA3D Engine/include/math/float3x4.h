...
public:
	...
	float3x4()																		{float v = ::sqrtf(-1.f); for ( int i = 0 ; i < ROWS ; ++i ) for ( int j = 0 ; j < COLUMNS ; ++j ) m[i][j] = v;}
#else
	float3x4()																		{}
#endif
	 ...
	 */
	void			setRotation( const math::float3& axis, float angle );
	...
	/** Returns top-right 3x1 submatrix. */
	void			getTranslation( math::float3* t ) const						{t->x=m[0][3]; t->y=m[1][3]; t->z=m[2][3];}

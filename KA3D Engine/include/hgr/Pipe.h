...
public:
	 ...
	 */
	virtual void	render( gr::Texture* target, gr::Context* context, Scene* scene, Camera* camera ) = 0;
...
protected:
	 ...
	 */
	static P(gr::Primitive)	createOverlayPrimitive( gr::Context* context, int width, int height );

	/**
	 * Renders viewport size overlay screen using specified shader and 
	 * optional on-fly set texture.
	 */
	static void				renderOverlay( gr::Primitive* overlay, gr::Shader* shader, gr::Texture* basetex=0 );
	 ...
	 * @param height Texture height to be rendered in pixels.
	 */
	static void				resizeOverlay( gr::Primitive* overlay, int width, int height );

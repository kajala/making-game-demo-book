...
bool String::endsWith( const String& suffix ) const
{
	...
	if ( otherlen <= thislen )
		return 0 == strncmp( Globals::get().stringPool.get(m_h)+thislen-otherlen, Globals::get().stringPool.get(suffix.m_h), otherlen );
	else
		return false;
}
...
bool String::startsWith( const String& prefix ) const
{
	...
	if ( otherlen <= thislen )
		return 0 == strncmp( Globals::get().stringPool.get(m_h), Globals::get().stringPool.get(prefix.m_h), otherlen );
	else
		return false;
}
...
int String::indexOf( const String& str, int index ) const
{
	...
	char*	s1			= Globals::get().stringPool.get(str.m_h);
	char*	s2			= Globals::get().stringPool.get(m_h);
...
bool String::regionMatches( int thisoffset, const String& other, int otheroffset, int length ) const
{
	...
	char*	s1			= Globals::get().stringPool.get(m_h);
	char*	s2			= Globals::get().stringPool.get(other.m_h);
...
String String::replace( char oldchar, char newchar ) const
{
	...
	if ( thislen > 0 )
	{
		...
		char* s = Globals::get().stringPool.get(str.m_h);
		char* s2 = Globals::get().stringPool.get(m_h);
...
String String::substring( int begin, int end ) const
{
	...
	if ( end > begin )
	{
		...
		memcpy( Globals::get().stringPool.get(str.m_h), Globals::get().stringPool.get(m_h)+begin, len*sizeof(char) );
	}
...
String String::toLowerCase() const
{
	...
	if ( thislen > 0 )
	{
		str.m_h = allocateString( thislen );
		char* s = Globals::get().stringPool.get(str.m_h);
		char* s0 = Globals::get().stringPool.get(m_h);
...
String String::operator+( const char* other ) const
{
	...
	if ( len > 0 )
	{
		...
		if ( m_h != -1 )
			strcpy( sz, Globals::get().stringPool.get(m_h) );
		strcat( sz, other );
	}
...
bool String::cat( char* buf, int bufsize, const String& str )
{
	assert( bufsize > 0 );
	if ( str.m_h != -1 )
		return String::cat( buf, bufsize, Globals::get().stringPool.get(str.m_h) );
	return true;
}

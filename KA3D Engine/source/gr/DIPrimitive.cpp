...
DIPrimitive::DIPrimitive( const VertexFormat& vf, int vertices, int indices ) :
	m_sysMemData( 0 ),
	m_usedBone( 0 ),
	m_vertices( 0 ),
	m_indices( 0 ),
	...
	const math::float4& scale, const math::float4& bias )
{
	...
	VertexFormat::copyData( dst, dstpitch, dstdf, data, VertexFormat::getDataSize(df), df, scale, bias, count );
}

void DIPrimitive::setVertexData( VertexFormat::DataType dt, int index, const float4* data, int count )
{
	setVertexData( dt, index, data, VertexFormat::DF_V4_32, count, float4(1.f,1.f,1.f,1.f), float4(0,0,0,0) );
}
...
void DIPrimitive::getVertexData( VertexFormat::DataType dt, int index, float4* data, int count ) const
{
	...
	VertexFormat::copyData( data, sizeof(float4), VertexFormat::DF_V4_32, src, pitch, df, float4(1.f,1.f,1.f,1.f), float4(0,0,0,0), count );
}
	...
	const float4x4* boneworldtm, int boneworldtmcount, SortBuffer& tmp )
{
	...
	std::sort( intbuffer, intbuffer+tricount, SortLess(floatbuffer) );
	...
	const float4x4* boneworldtm, int boneworldtmcount, SortBuffer& tmp )
{
	...
	getTriangleDistances( refpos, worldtm, boneworldtm, boneworldtmcount, intbuffer, floatbuffer, tricount );
	std::sort( intbuffer, intbuffer+tricount, SortGreater(floatbuffer) );
	assert( floatbuffer[intbuffer[tricount-1]] <= floatbuffer[intbuffer[0]] );
	reorderTriangles( intbuffer, intbuffer+tricount );
}
...
void DIPrimitive::getTriangleDistances( const math::float3& worldpos, const math::float4x4& worldtm, const math::float4x4* boneworldtm, int boneworldtmcount, uint16_t* trix, float* tridist, int tricount ) const
{
	...
	Array<int>&	faceset1, Array<int>& faceset2 ) const
{
	...
	while ( faceset2.size() > 0 )
	{
		...
		for ( int i = 0 ; i < faceset2.size() ; ++i )
		{
			...
			facebones.resize( std::unique(facebones.begin(),facebones.end())-facebones.begin() );

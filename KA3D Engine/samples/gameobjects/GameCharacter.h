...
public:
	...
	GameCharacter( lua::LuaState* luastate, GameLevel* level, const lang::String& scriptname );
	...
	void				setAnimationDirectory( lang::String dir );
	...
	math::float3		ambient() const;

	static const char*	toString( BodyState bodystate );

	static const char*	toString( UpperBodyState bodystate );

	static 	BodyState		toBodyState( const lang::String& bodystate );

	static UpperBodyState	toUpperBodyState( const lang::String& bodystate );
...
private:
	...
	math::float3					m_amb0[8];
	int								m_ambIndex;
	math::float3					m_ambTarget;
	math::float3					m_ambSpeed;
	math::float3					m_amb;

	// body animations
	lang::String					m_animDir;
	...
	// weapons
	lang::Array<P(GameWeapon)>		m_weapons;
	bool							m_readyToFire;

	P(hgr::TransformAnimationSet)	loadCharacterAnimationImpl( const lang::String& filename, hgr::TransformAnimation::BehaviourType endbehaviour=hgr::TransformAnimation::BEHAVIOUR_REPEAT );
	void							loadCharacterAnimation( lang::String bodystate, lang::String filename, lang::String endbehavior );
	void							loadUpperBodyAnimation( lang::String bodystate, lang::String direction, lang::String filename, lang::String endbehavior );
	...
	math::float3					getLocalVelocityFromBodyAnimation( hgr::TransformAnimationSet** anims, const float* animtimes, const float* animweights, int animsets );
	...
	void							updateVelocity( float dt, const math::float3& localvel );

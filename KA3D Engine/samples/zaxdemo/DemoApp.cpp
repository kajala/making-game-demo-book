...
void DemoApp::keyDown( KeyType key )
{
	...
	else if ( KEY_F11 == key )
	{
		...
		else
		{
			...
			FileInputStream filein( "gamestate.temp" );
			DataInputStream datain( &filein );
			serialize( &datain, 0 );

			playInput( "input.temp" );
		}
	}
	else if ( KEY_F12 == key && !playingInput() )
	{
		if ( recordingInput() )
		{
			stopRecordingInput();
		...
		{
			FileOutputStream fileout( "gamestate.temp" );
			DataOutputStream dataout( &fileout );
			serialize( 0, &dataout );

			recordInput( "input.temp" );
		}
	}
	else if ( KEY_F9 == key )
	{
		m_recordAnim = !m_recordAnim;
	}
	else if ( KEY_F4 == key )
	{
		toggleTimeScale();
